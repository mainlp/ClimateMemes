...
def initialize_model_and_processor():
    ...
    model_path = "/nfs/gdata/mistral_models/7B-Instruct-v0.3"
...
def get_model_interpretation(image_name, n_shot):
    model_interpretation_df = pd.read_excel(f'results/interpretation/llava_{n_shot}_shot_interpretation.xlsx')
    # Extract the image name without URL prefix
    image_name_without_prefix = os.path.basename(image_name)
...
def generate_stance(model, n_shot, references, meta, ocrs=None, interpretations=None):
    """
    ...
    answer = model(conversation)[0]['generated_text'][-1]['content']
    # print("A:", answer)
    answers.append(answer)

    return answers

def generate_frame(model, n_shot, references, options, meta, ocrs=None, interpretations=None):
    """
    Generate answers using the model with n-shot examples, including correct answers as reference.
...
def save_answers_to_excel(image_folder, output_file, model, n_shot, similarity_df, gold_label_df, task, order=None, options=None, meta=None):
    """
    ...
    for image_name in tqdm(os.listdir(image_folder), desc="Processing images"):
        if image_name.lower().endswith(('.png', '.jpg', '.jpeg')):
            ...
            if task == "frame":
                ...
                answers = generate_frame(model, n_shot, references, options, meta, ocrs, interpretations)
            
            elif task == "stance":
                # Generate answers with n-shot references
                answers = generate_stance(model, n_shot, gold_stances, meta, ocrs, interpretations)
...
def main():
    ...
    if args.task == "frame":
        ...
        for order, options in enumerate(rotated_options):
            output_file = f'./results/{args.task}/Mistral_{args.n_shot}_shot_{"_".join(args.meta)}_order{order}.xlsx'
        ...
        output_file = f'./results/{args.task}/Mistral_{args.n_shot}_shot_{"_".join(args.meta)}.xlsx'
